...
public class OrganalysisController implements OrganalysisApi {
    ...
    @Override
    public ResponseEntity<List<Object>> recommendOrganizationsByCoordinates(CoordinatesDTO coordinatesDTO, Long amount, OrganizationTypeDto organizationTypeDto) {
        if (coordinatesDTO == null) throw new NullArgsInRequestException("id");
        if (amount == null) throw new NullArgsInRequestException("amount");
        if (organizationTypeDto == null) throw new NullArgsInRequestException("organizationType");
